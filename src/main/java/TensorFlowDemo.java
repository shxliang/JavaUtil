...
public class TensorFlowDemo {
    public static void main(String[] args) {
        ...
        String imageFile = "D://lsx/test.png";

//        byte[] graphDef = readAllBytesOrExit(Paths.get(modelDir, "sexy_inception_v4_freeze.pb"));
        byte[] graphDef = readAllBytesOrExit(Paths.get(modelDir, "tensorflow_inception_graph.pb"));
//        List<String> labels = readAllLinesOrExit(Paths.get(modelDir, "sexy_slim_labels.txt"));
    ...
    private static float[] executeInceptionGraph(byte[] graphDef, Tensor<Float> image) {
        try (Graph g = new Graph()) {
            ...
            try (Session s = new Session(g);
                 Tensor<Float> result = s.runner().feed("input", image).fetch("InceptionV4/Logits/Predictions").run().get(0).expect(Float.class)) {
